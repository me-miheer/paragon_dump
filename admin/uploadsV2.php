...
$expectedHeaders = ['Article', 'Size', 'Scheme'];
$response = [];
...
if (isset($_FILES['excelFile']) && $_FILES['excelFile']['error'] == 0) {
    ...
    if ($fileType == 'csv') {
        ...
        if (($handle = fopen($fileTmpPath, "r")) !== FALSE) {
            ...
            if (strtolower($headers[0]) == strtolower($expectedHeaders[0]) && strtolower($headers[0]) == strtolower($expectedHeaders[0]) && strtolower($headers[0]) == strtolower($expectedHeaders[0])) {
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    ...
                    try {
                        ...
                        if(empty(trim($data[0])) || empty(trim($data[1])) || empty(trim($data[2]))){
                            ...
                            $txt[] = "Error: Any of three cells is empty.";
                ...
                $response[] = array(
                    "status" => false,
                    "message" => "Error: Incorrect file format. Expected headers: ['Article', 'Size', 'Scheme']."
                );
            }
    ...
    <div class="section bg-info" style="position: fixed; width: 100vw; height: 100vh; margin: 0; padding: 0; top: 0; left: 0; right: 0; bottom: 0; display: flex; justify-content: center; align-items:center;">
        <div class="data card">
            <div class="card-header">
                <i class="bi bi-arrow-left" width="50px" height="50px" onclick="location.replace('articleV2.php')"></i>
            </div>
            ...
            <div class="card-header">
                <button class="btn btn-success bg-gradient w-100 p-2"  onclick="location.replace('articleV2.php')">Aknowledged</button>
            </div>
        </div>
    </div>
