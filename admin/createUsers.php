...
if (isset($_GET['action'])) {
    ...
    if ($action == 'saveUser') {
        ...
        if ($id) {
            // Update user (cannot change password)
            $sql = "UPDATE user SET 
...
<div class="container py-4">
    ...
    <button class="btn btn-primary mb-3" onclick="openForm()">+ Create User</button>
...
<div class="modal" id="userModal">
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="userForm">
        <div class="modal-header"><h5 class="modal-title">User Form</h5></div>
        <div class="modal-body">
          ...
          <!-- Dealer hidden -->
          <input type="hidden" name="dealer" id="dealer" value="N">
...
function loadUsers(page=1, search='') {
    $.get("createUsers.php?action=getUsers", {page:page, search:search}, function(res){
        let data = JSON.parse(res);
        let html = `<table class="table table-bordered"><tr>
            <th>ID</th><th>Name</th><th>Email</th><th>Mobile</th><th>Shop</th><th>Town</th><th>Retail Type</th><th>User Type</th><th>Action</th></tr>`;
        data.users.forEach(u=>{
            html += `<tr>
                <td>${u.id}</td><td>${u.name}</td><td>${u.email}</td>
                <td>${u.mobile}</td><td>${u.shop}</td><td>${u.town}</td><td>${u.retailType}</td><td>${u.user_type}</td>
                <td>
...
function openForm(){
    ...
    $("#user_type").val('user');
    $("#password").show(); // show password for creation
...
function deleteUser(id){
    if(confirm("Delete this user?")){
        $.post("createUsers.php?action=deleteUser",{id:id},function(){
            loadUsers(currentPage);
        });
    }
}
...
function resetPassword(id){
    if(confirm("Reset password to 123456?")){
        $.post("createUsers.php?action=resetPassword",{id:id},function(res){
            alert(JSON.parse(res).message);
        });
    }
}

$("#search").on("keyup", function(){ loadUsers(1, $(this).val()); });
